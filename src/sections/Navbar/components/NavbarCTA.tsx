...
export const NavbarCTA = () => {
  return (
    <a
      href="/contact"
      onClick={(event) => handleSpaNavigation(event, "/contact")}
      className="rd-navbar_btn relative text-zinc-50 text-base font-bold items-center bg-violet-600 box-border caret-transparent flex h-16 justify-center justify-self-center tracking-[0.32px] leading-6 max-w-full min-h-[auto] min-w-[auto] translate-y-[-60.0%] w-16 rounded-[18px] md:h-auto md:justify-self-auto md:transform-none md:w-auto md:rounded-lg"
    >
      <div className="rd-navbar_btn_bg absolute bg-violet-600 bg-[conic-gradient(at_8.04%_51.79%,rgb(251,250,246)_0deg,rgba(251,250,246,0)_360deg),none] bg-size-[auto,auto] box-border caret-transparent pointer-events-none bg-[position:0%,0%_0%,0%] inset-[0%]"></div>
      <div className="rd-navbar_btn_content relative items-center bg-neutral-950 box-border caret-transparent gap-x-2 flex h-full justify-center min-h-[auto] min-w-[auto] gap-y-2 w-full z-0 p-[5px] md:px-8 md:py-4">
        ...
        <img
          ...
          alt="Icon"
          className="rd-navbar_btn_icon rd-navbar_btn_shape text-[32px] box-border caret-transparent shrink-0 h-8 leading-[48px] align-baseline w-8 md:text-2xl md:h-6 md:leading-9 md:w-6"
        />
        <span className="rd-navbar_btn_text box-border caret-transparent hidden min-h-0 min-w-0 md:block md:min-h-[auto] md:min-w-[auto]">
          Start a Project
        </span>
        <img
